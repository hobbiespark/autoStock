...
1. **F1→F2 검증 인프라** (지금, 장 시간 무관): 부트스트랩 엔진 → SPA → 기존 10계열에 병기 수치 산출
2. **ADR-12 사용자 승인/기각 결정** → 승인 시 F4 재판정 1회
3. **장중 실측** (개장 시간): ws_probe REAL FID·체결통보 → RealMessageParser/OrderNoticeHandler 반영 → RUNBOOK 5단계 모의 무인 운영 개시(트랙 C)
4. **E2 공모주 반자동** (DART 키 발급 후): 캘린더·필터·알림
5. E4 익절/손절 paper A/B는 모의 운영 안정화 후 / 스윙(감성)은 Phase 7
