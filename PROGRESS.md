...
C2-1. **운영 1일차(2026-09-11) 실전 발견·수정 로그**:
   ...
   - 🧹 잔여 정리(SQL 1건): BUY-004의 filled_quantity가 이중계상 시절 값 23으로 저장됨(실체결 19) — `UPDATE orders SET filled_quantity=19 WHERE client_order_id LIKE '%BUY-004';`
...
G3. **F-Score 유니버스 필터 (M2 가설)** — fnlttSinglAcntAll 분기 산출, 기존 계열 유니버스 개선, 게이트 v2
   - ⚠️ 수집 운영 교훈(2026-09-11): 호스트에서 8스레드 무스로틀 수집 → +800건 지점부터 전건 실패(성공 615 동결) → **DART IP 차단**(응답 없이 연결 종료, ~24h). 조치: 스크립트에 전역 스로틀(초당 6건=분당 360) + 한도 응답(020/021/800/901)은 실패 기록 없이 즉시 중단 + 차단 중 오염된 .fail_2024 삭제 후 재시도. fs_2024 현황: 성공 ~1,292(677+615), 재시도 대상 ~2,700. 내일 재개
