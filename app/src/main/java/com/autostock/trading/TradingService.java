 ...
 * (ADR-6 재편으로 execution → trading 모듈 이동, 클래스명 ExecutionService → TradingService.
 * trading은 "주문 생성·상태 관리·대사"를, execution은 "브로커 전달"만 책임진다 —
 * ARCHITECTURE.md 2절).
 *
 * <p>두 가지 실행 모드 (application.yml의 execution.mode — yml 호환성을 위해 프로퍼티
 * 프리픽스는 execution을 그대로 유지한다, {@link TradingProperties} 참고):
 * <pre>
