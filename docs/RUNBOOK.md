...
- [ ] PostgreSQL 기동 — 원클릭: `.\scripts\setup_docker.ps1` (Docker 설치/기동 확인 → postgres up → healthy 대기 → 접속 스모크까지 자동. Docker 미설치 시 `winget install -e --id Docker.DockerDesktop` 안내 출력). 수동으로 하려면: `docker compose -f infra/docker-compose.yml up -d postgres`
